...
class Repo(BaseRepo):
    ...
    async def add_user(self, user_id: int, full_name: str, username: str | None) -> User:
        stmt = insert(User).values(id=user_id, full_name=full_name, username=username).on_conflict_do_nothing().returning(User)
    ...
    async def get_goods(self, query: str) -> Sequence[Good]:
        return await self.scalars_all(select(Good).where((Good.id.ilike(f"%{query}%") | (Good.name.ilike(f"%{query}%")))))

    async def get_user_by_request(self, request: str) -> Sequence[User]:
        return await self.scalars_all(select(User).where(User.full_name.ilike(f"%{request}%") | User.username.ilike(f"%{request}%") | (cast(User.id, VARCHAR)).ilike(f"%{request}%")).limit(50))
